...
Engram follows [SemVer 2.0.0](https://semver.org/): breaking schema or MCP
changes bump the major; new tools or event types bump the minor; bug fixes bump
the patch. While on the `0.x` series the major is held at 0, so the event-log
schema, the on-disk layout under `~/.engram/`, and MCP tool signatures may still
change between minor versions with no backward-compatibility guarantees.
Migrations are version-gated via `schema_version`; `init_schema()` applies any
`schema/NNN_*.sql` past the highest applied version on every connect. Releases
live as git tags on `main`.
